...
The result? A continuous underline that runs through all the text, regardless of the different styling applied to the `<strong>` and `<em>` elements. The underline doesn't stop, restart, or change color—it's a single, unbroken line.
...
```html
<!-- Instead of this -->
<p class="underlined">Text with <span>nested content</span> here.</p>
